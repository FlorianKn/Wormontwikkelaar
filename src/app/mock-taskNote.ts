import { TaskNote } from './task-note';

export const Status: String[] = ['Open', 'InProgress', 'Finished'];
export const Type: String[] = ['Task', 'Note'];

export const TasksNotes: TaskNote[] = [
    { title: 'Lorem ipsum dolor sit amet, consectetuer', description: 'Lorem ipsum dolor sit amet, consectetuer adipiscing elit.'
     + 'Aenean commodo ligula eget dolor. Aenean massa.', technicianId: 1, type: Type[0],
      creationDate: new Date(2018, 11, 24, 10, 33), status: Status[0]},
     { title: 'Lorem ipsum dolor sit amet, consectetuer', description: 'Lorem ipsum dolor sit amet, consectetuer adipiscing elit.'
     + 'Aenean commodo ligula eget dolor. Aenean massa.', technicianId: 1, type: Type[1],
      creationDate: new Date(2018, 10, 12, 10, 5), status: Status[1]},
     { title: 'Lorem ipsum dolor sit amet, consectetuer', description: 'Lorem ipsum dolor sit amet, consectetuer adipiscing elit.'
     + 'Aenean commodo ligula eget dolor. Aenean massa.', technicianId: 1, type: Type[0],
      creationDate: new Date(2018, 12, 2, 13, 37), status: Status[1]},
     { title: 'Lorem ipsum dolor sit amet, consectetuer', description: 'Lorem ipsum dolor sit amet, consectetuer adipiscing elit.'
     + 'Aenean commodo ligula eget dolor. Aenean massa.', technicianId: 1, type: Type[1],
      creationDate: new Date(2018, 11, 30, 8, 36), status: Status[1]},
     { title: 'Lorem ipsum dolor sit amet, consectetuer', description: 'Lorem ipsum dolor sit amet, consectetuer adipiscing elit.'
     + 'Aenean commodo ligula eget dolor. Aenean massa.', technicianId: 1, type: Type[0],
      creationDate: new Date(2018, 10, 10, 10, 10), status: Status[2]}
];
